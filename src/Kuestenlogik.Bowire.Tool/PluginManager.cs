...
/// <summary>
/// The <c>bowire plugin</c> verbs: install, download, list, update,
/// uninstall, inspect. Plugins live in <c>~/.bowire/plugins/{packageId}/</c>.
/// </summary>
/// <remarks>
/// Static on purpose, and only these verbs. Each one is a function of a
/// package id, a resolved directory and a writer pair — no state, no
/// lifetime, nothing to own. Their nine callers are System.CommandLine
/// action lambdas that see a <c>ParseResult</c> and an
/// <c>InvocationConfiguration</c> and have no container in reach, and the
/// command tree is built even for <c>bowire --help</c>, so an injected
/// instance would be constructed for invocations that never touch it.
/// <para>
/// Loading plugins is the part that owns state, and it moved out to
/// <see cref="Plugins.IBowirePluginLoader"/> in #546.
/// </para>
/// </remarks>
