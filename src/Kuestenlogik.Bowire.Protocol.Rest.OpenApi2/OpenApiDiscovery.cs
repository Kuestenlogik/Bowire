...
public static class OpenApiDiscovery
{
        ...
        string docUrl, HttpClient http, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Same explicit YAML-reader registration as ParseRawAsync —
            // Microsoft.OpenApi 2.x doesn't auto-register and would
            // silently return null on YAML input.
            var settings = new Microsoft.OpenApi.Reader.OpenApiReaderSettings();
            settings.AddYamlReader();
            var readResult = await OpenApiDocument.LoadAsync(stream, format: null, settings: settings, ct).ConfigureAwait(false);
    ...
    public static async Task<DiscoveredApi?> ParseRawAsync(string content, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Microsoft.OpenApi 2.x doesn't auto-register YAML readers
            // when format == null + settings == null. Without an
            // explicit AddYamlReader() the YAML path silently returns
            // a null document. Register it here so callers can pass
            // either JSON or YAML uniformly.
            var settings = new Microsoft.OpenApi.Reader.OpenApiReaderSettings();
            settings.AddYamlReader();
            var readResult = await OpenApiDocument.LoadAsync(stream, format: null, settings: settings, ct).ConfigureAwait(false);
            if (readResult?.Document is null) return null;
