...
- [x] **AsyncAPI 2.x mapping** — `MapV2Channels` walks `channels[].publish` + `channels[].subscribe` (V2's inline-operation shape) into the same `BowireServiceInfo` topology V3 produces. Channel.Subscribe → asyncapi-receive (we receive), Channel.Publish → asyncapi-send (we send). Operation-id used as method name when set, else "publish" / "subscribe" fallback. V2 invocation routing through the binding resolvers is a separate item — currently V2 docs discover but don't invoke.
