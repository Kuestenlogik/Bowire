...
Phase A3's MQTT resolver works end-to-end *without* the document declaring a `bindings:` block. As soon as `bindings.mqtt.qos: 2` (or any unquoted version like `asyncapi: 3.0.0`) is present, the Neuroglia SDK reader throws — its `StringEnumDeserializer` asks YamlDotNet for a `Decimal`-typed scalar, the YAML implicit-type resolver already classified the value as int / string, `Decimal.Parse` blows up. Not a Bowire bug, not a Bowire/Neuroglia interface bug, not a Bowire-MQTT-plugin bug: a layer-below issue in `Neuroglia.Serialization.YamlDotNet`'s `StringEnumDeserializer`. Repro + draft GitHub issue tracked in [`docs/architecture/asyncapi-net-sdk-issue-draft.md`](docs/architecture/asyncapi-net-sdk-issue-draft.md).

- [ ] **File the upstream issue** at https://github.com/asyncapi/net-sdk/issues — use the draft above verbatim. Link the issue # back into this entry once filed.
...
- [ ] **Bindings-detail extraction** — read the binding-specific fields (`bindings.mqtt.qos`, `.retain`, `.topic`; `bindings.kafka.partition`, `.key`; `bindings.ws.subprotocol`) via either the SDK once the bug is fixed, or our own light YAML walking. Populate `AsyncApiChannelContext.BindingFields` so resolvers can use them instead of defaults.
- [ ] **MQTT broker integration test** — once binding fields land, run a smart-home doc through the loader → MqttBindingResolver → publish + subscribe against a Testcontainers-spawned Mosquitto. Sits in `Kuestenlogik.Bowire.IntegrationTests`.
- [ ] **AsyncAPI 2.x mapping** — the SDK reader already produces `V2AsyncApiDocument` instances; today they get dropped in `BowireAsyncApiProtocol.DiscoverAsync`. Add `MapV2Channels` so docs written against the older spec (which is still the majority in the wild) discover too.
