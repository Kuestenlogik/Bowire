...
- [ ] **Phase 2b — Matcher → AttackPredicate translation.** `status` matchers → `StatusEquals` predicate. `word` matchers → `BodyContains` / `BodyContainsAny` depending on `condition: and/or`. `regex` matchers → `BodyMatches`. Multi-matcher composition via `matchers-condition: and/or` lands on `AllOf` / `AnyOf`. `negative: true` wraps in `Not`. Coverage: ~70% of the corpus's HTTP templates.
- [ ] **Phase 2c — Variable substitution.** Resolve Nuclei's `{{BaseURL}}`, `{{Hostname}}`, `{{RandStr_N}}` placeholders against the scan target at probe time. Hooks into ScanCommand's existing variable layer; no new probe-runner needed.
- [ ] **Phase 2d — Multi-step + payload matrices.** `http[]` arrays with multiple entries map to multi-step recordings. `payloads:` blocks fan out into one probe per payload value (Nuclei calls this "fuzzing matrix"). Coverage climbs to ~85% of the corpus.
- [ ] **Phase 2e — ScanCommand integration.** New `--corpus-nuclei <dir>` flag on `bowire scan`. Reads + converts every template in the directory, runs them through the existing scan pipeline alongside the operator's `--corpus`. Marketing-relevant moment: "8000+ templates via Nuclei" becomes a true claim end-to-end.
