...
- [ ] **AMQP** (`Kuestenlogik.Bowire.Protocol.Amqp`) — AMQP 0.9.1 + 1.0 via `RabbitMQ.Client` + `AMQPNetLite`. Single plugin id, two wires picked by URL scheme (`amqp://` / `amqps://` → 0.9.1; `amqp1://` / `amqps1://` → 1.0). Discovery via RabbitMQ Management HTTP API for 0.9.1; synthetic `Broker` service for 1.0 (no broker-side schema concept). **Skeleton released as [`v0.1.0`](https://github.com/Kuestenlogik/Bowire.Protocol.Amqp/releases/tag/v0.1.0)** — `IBowireProtocol` surface pinned, all async methods throw `NotImplementedException` on purpose so the skeleton state is visible. Discovery + invocation + streaming land in `0.2.x`.
...
- [ ] **Phase C — AMQP / NATS / SNS-SQS bindings** — `bindings.amqp` + `bindings.amqp1` gated on `Kuestenlogik.Bowire.Protocol.Amqp` 0.2.x (skeleton already published as v0.1.0); `bindings.nats` gated on a future NATS wire plugin; `bindings.sns` / `sqs` gated on a future SNS-SQS plugin (no AWS-specific plugin scaffolded yet). The loader stays the same; each binding adds a translation case.
