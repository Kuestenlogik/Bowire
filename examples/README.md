# Bowire Examples — per-plugin demo servers

Minimal, runnable servers for every Bowire protocol plugin. Each example
boots a target Bowire can connect to so you can see the plugin in
action without standing up a real backend.

> **Looking for the unified-domain reference apps instead?**
> The [`Kuestenlogik/Bowire.Samples`](https://github.com/Kuestenlogik/Bowire.Samples)
> repo holds the same Harbor Control Center domain implemented across
> every protocol — useful for protocol-comparison work and the marketing-
> site captures. The examples here use protocol-canonical domains
> (PetStore for REST, Greeter for gRPC, Northwind for OData, &c) and
> stay deliberately small. Pick this folder if you want to test a
> specific plugin; pick `Bowire.Samples` if you want to see Bowire
> render the same business model through different wire formats.

> _(Renamed from `samples/` to `examples/` in 2026-06 to disambiguate
> from the [`Bowire.Samples`](https://github.com/Kuestenlogik/Bowire.Samples)
> sibling repo. Existing GitHub links to the old path should redirect
> via the repo's auto-history; bookmarks under `tree/main/samples/X` won't.)_
