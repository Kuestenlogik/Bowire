...
- [~] **AI security-scan orchestration (#104).** The four AI primitives (threat-model #59, template-suggest #60, triage #61, fuzz-values #62) plus the report (#107) exist as separate actions; #104 is the single flow that composes them: threat-model rank → probe each above-threshold endpoint → triage every finding (keep real / suppress false-positive) → report. Shipped: the deterministic orchestration control flow (`SecurityScanOrchestrator` + the `ISecurityScanSteps` seam), plus the concrete `AiSecurityScanSteps` adapter and `POST /api/ai/security-scan` endpoint — an `IChatClient` drives the threat-model + triage + report-summary stages and the probe stage runs through the `ISecurityScanProbeRunner` seam. Every AI stage degrades to a deterministic default (threat-model → a path heuristic, triage → keep-by-default, report → the deterministic markdown), so the pipeline always completes with or without a model. The scanner-side `ISecurityScanProbeRunner` (`ScannerProbeRunner`) now supplies live probe execution — it runs the HTTP OWASP probes + built-ins against each endpoint URL and maps the vulnerable findings back for triage; the standalone workbench registers it via `AddBowireSecurityScanProbeRunner()` (embedded hosts opt in, else plan-only). Follow-up: the workbench one-button surface.
- [~] **AI OWASP API Top 10 panel (#106).** Per method, map the ten entries to a tri-state status (at-risk / maybe / n-a) + a concrete probe. Shipped: the deterministic `OwaspApiTop10Mapper` (rule-based signals on path / verb / request fields — id-in-path → API1 BOLA, write-with-body → API3 mass-assignment, admin path → API5 BFLA, URL field → API7 SSRF, …) + `POST /api/ai/owasp-panel`, which returns the deterministic rows and adds a short AI review of the method's posture when a model is connected (degrading to the rows alone otherwise). Follow-up: the workbench per-method panel surface.
