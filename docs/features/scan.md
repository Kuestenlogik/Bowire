...
Beyond HTTP, the suite also runs **protocol-specific probes** that drive a protocol plugin's own invoke path — **GraphQL introspection** and **gRPC server reflection** exposed anonymously, both rolling up to API9 as the protocol analog of an exposed API inventory. They are discovery-only and run only when the matching protocol plugin is deployed (the `bowire` tool ships them); an absent plugin or a non-matching target skips cleanly. See the [OWASP suite reference](../security/owasp-api/index.md#protocol-specific-probes).
