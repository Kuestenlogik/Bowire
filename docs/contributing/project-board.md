...
| **Release** *(single-select)* | The **cross-repo grouping** the roadmap is bucketed by ([`generate-roadmap.mjs`](../../scripts/ci/generate-roadmap.mjs)). Since 2026-09-19 its planned options are the **ordered work sections** `M<n> — <theme>` (see [Milestones and releases](#milestones-and-releases)); a shipped option keeps the version it shipped as (`v2.7`, …). It spans **every repo** on the board, which a native (repo-scoped) `Milestone` can't: a sibling issue (Akka, Dis, …) rides the same section as main-repo work. It's a grouping key — it has **no lifecycle** (see `Milestone`), and it is **independent of `Status`**: an item can sit in `M2` and still rest in `Backlog`. Required once the item leaves `Backlog` — see [Release is mandatory](#release-is-mandatory). |
...
- **Layout**: Roadmap
- **Group by**: `Release` (the section). Since the siblings mirror the section as their own milestone, grouping by `Milestone` shows the same picture, split per repo.
- **Filter**: `Status` ≠ `Done`
...
**Two axes, kept apart:**
- **`Release` (field)** → planning / roadmap grouping: the section (`M<n> — <theme>`) while planned, the version once shipped. Spans repos. *No lifecycle* — it's a grouping key.
