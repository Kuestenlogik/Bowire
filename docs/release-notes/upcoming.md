...
The library is seven megabytes inflated, so it is fetched only once a frame actually carries a geometry, and it ships gzipped: the extension asset endpoint (`/api/ui/extensions/{id}/{name}`) now serves an asset declared with a `.gz` suffix under its plain leaf — passed through with `Content-Encoding: gzip` for a client that accepts it, inflated for one that does not, `Vary: Accept-Encoding` on both — so the `Kuestenlogik.Bowire.Map` package carries the 1.36 MB the wire carries. The TacticalAPI sample gained an overlay of seven control measures, one per `SymbolLocation` case, as the data to see this against.
