...
294 commits since v2.0.1. One small breaking change — the standalone `Kuestenlogik.Bowire.Rail.Collections` package is retired; embedded hosts that referenced it explicitly need to drop the reference. See **Breaking changes** below. Existing workspaces, collections, recordings, benchmarks load identically.
...
### Help has its own rail now

Help moved out of the right-side drawer into a full rail with the
standard left-sidebar + main-pane shape every other rail uses.
Browsing + searching topics has the workbench's room to breathe;
the Assistant drawer stops width-jumping every time you switch
tabs. Every topic has an "Open in new tab" icon so a multi-monitor
operator can keep docs visible while working. The rail is
contributed by the `Kuestenlogik.Bowire.Help` package — hosts that
don't reference it don't see the rail (same pluggable contract as
the Map extension).
