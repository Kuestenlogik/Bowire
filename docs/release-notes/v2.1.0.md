...
`Rail modes` row renames to `Rails`; `Modules`, `Plugins`,
`Assistant`, and `Discovery` collapse into a single `Extensions`
group; and `Data` moves under `Workspace…` because everything it
manages is workspace-scoped. Internal tab ids (`rails`, `modules`,
`ai`, `discovery`, `data`) keep verbatim so saved deep links still
resolve.
