...
async function main() {
    ...
    try {
        ...
        // Track the start page index in the merged PDF for each
        // TOC_ENTRIES.file as we go. Drives the PDF outline tree
        // (Acrobat / Preview / Sumatra side-panel bookmarks) so
        // the reader can jump between top-level sections without
        // scrolling — closes #241's "no way to navigate" complaint.
        const sectionStartPages = new Map();
        let count = 0;
        let tocInserted = false;
        for (const rel of pages) {
            // 0-based merged-page index where THIS rel will land.
            const startIndex = merged.getPageCount();
            if (TOC_ENTRIES.some(e => e.file === rel)) {
                sectionStartPages.set(rel, startIndex);
            }
